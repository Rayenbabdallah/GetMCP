...
- **Compromised database** — the audit chain is tamper-*evident*, not tamper-*proof*. An attacker with write access to Postgres can rewrite both the `AuditLog` row and the matching `Organization.lastAuditHash` head pointer. To get tamper-proof, periodically anchor `(orgId, lastAuditSeq, lastAuditHash)` to an external append-only store (object lock, transparency log, customer's own infra).
